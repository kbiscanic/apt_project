...
%TODO jagarov komentar - ja stvarno mislim da je ovo u 2. dijelu uvoda predetaljno,
% "Introduction should not be too elaborate, that is what other sections are for"

\section{Overview of the field}

% short overview of the field: similar algorithms and systems

Semantic textual similarity is discussed at length in \citep{literatura1}. The system described there directly inspired the implementation of our system.
...
We evaluated our model using Pearson and Spearman correlation coefficients. The performance on test sets is shown in Table~\ref{tab:test-results-table}. Aggregate performance according to three aggregate evaluation measures proposed in \citep{literatura2} are shown in Table~\ref{tab:all-results-table}.
